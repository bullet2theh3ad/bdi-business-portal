-- Fix Alec's auth_id mismatch with correct column names

-- STEP 1: Check current organization memberships for Alec (using correct column names)
SELECT 
...
-- STEP 5: Final login verification for Alec
SELECT 
  'ALEC FINAL LOGIN CHECK' as status,
  u.email,
  u.name,
  u.role,
  u.is_active,
...
-- STEP 6: Check all admin users for similar auth sync issues
SELECT 
  'ALL ADMIN AUTH SYNC CHECK' as check_type,
  u.email,
  u.name,
