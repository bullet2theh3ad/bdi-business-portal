...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedPurchaseOrders = (purchaseOrdersData || []).map((row: any) => ({
      ...
      requestedDeliveryWeek: row.requested_delivery_week,
      status: row.status,
      terms: row.terms,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const purchaseOrderData = {
      ...
      requestedDeliveryWeek: formData.get('requestedDeliveryWeek') as string,
    ...
    const { data: newPurchaseOrder, error: insertError } = await supabase
      ...
      .insert({
        ...
        requested_delivery_week: purchaseOrderData.requestedDeliveryWeek,
