...
export async function GET(request: NextRequest) {
  try {
    ...
    const [customersCount, invoicesCount, vendorsCount, expensesCount, itemsCount, paymentsCount, billsCount, salesReceiptsCount, creditMemosCount, posCount, depositsCount, billPaymentsCount] = await Promise.all([
      ...
      supabaseService.from('quickbooks_deposits').select('id', { count: 'exact', head: true }),
      supabaseService.from('quickbooks_bill_payments').select('id', { count: 'exact', head: true }),
    ]);
    ...
    return NextResponse.json({
      stats: {
        ...
        billPayments: billPaymentsCount.count || 0,
      },
