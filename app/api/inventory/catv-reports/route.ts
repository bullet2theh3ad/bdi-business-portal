...
export async function POST(request: NextRequest) {
  try {
    ...
    // Debug the categorization totals
    topSkus.slice(0, 3).forEach((sku: any) => {
      console.log(`📊 ${sku.model} breakdown: IN=${sku.receivedIn}, RMA=${sku.rmaOut}, EMG=${sku.shippedEmgOut}, WIP=${sku.wipInHouse}`);
    });
