...
/**
 * GET /api/gl-management/weekly-opex
 * Fetch all GL transactions categorized as OpEx and aggregate by week
 * 
 * Query params:
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`💼 [Weekly OpEx] Fetching OpEx data from ${startDate || 'all'} to ${endDate || 'all'}`);
    ...
    // Fetch OpEx overrides (bills and expenses categorized as opex)
    let overridesQuery = supabaseService
      .from('gl_transaction_overrides')
      .select('*')
      .eq('override_category', 'opex');

    const { data: opexOverrides, error: overridesError } = await overridesQuery;

    if (overridesError) {
      console.error('Error fetching OpEx overrides:', overridesError);
      return NextResponse.json({ error: 'Failed to fetch OpEx overrides' }, { status: 500 });
    }

    console.log(`💼 [Weekly OpEx] Found ${opexOverrides?.length || 0} OpEx overrides`);

    // Build override map for quick lookup - FIXED: use transaction_id instead of transaction_key
    const overrideMap = new Map();
    (opexOverrides || []).forEach((override: any) => {
      overrideMap.set(override.transaction_id, override);
    });
    
    // Debug: Show sample override keys
    const sampleKeys = Array.from(overrideMap.keys()).slice(0, 5);
    console.log(`🔑 [Weekly OpEx] Sample override keys:`, sampleKeys);
    ...
    console.log(`📄 [Weekly OpEx] Found ${bills?.length || 0} bills`);
    ...
    console.log(`💳 [Weekly OpEx] Found ${expenses?.length || 0} expenses`);

    // Fetch QuickBooks Payments (OpEx often comes from payments)
    ...
    console.log(`💰 [Weekly OpEx] Found ${payments?.length || 0} payments`);
    ...
    const addToWeekly = (
      ...
      if (lowerAccountType.includes('contract labor')) {
        weekData.breakdown.contractLabor += amount;
      } else if (lowerAccountType.includes('consulting')) {
        weekData.breakdown.consultingServices += amount;
      } else if (lowerAccountType.includes('travel')) {
        weekData.breakdown.travel += amount;
      } else if (lowerAccountType.includes('software') || lowerAccountType.includes('subscription')) {
        weekData.breakdown.software += amount;
      } else {
    ...
    console.log(`✅ [Weekly OpEx] Processed ${billsProcessed} bills, ${billsWithOpEx} had OpEx overrides`);

    // Process Expenses
    let expensesProcessed = 0;
    let expensesWithOpEx = 0;
    ...
    console.log(`✅ [Weekly OpEx] Processed ${expensesProcessed} expenses, ${expensesWithOpEx} had OpEx overrides`);

    // Process Payments (OpEx often comes from payments for services, subscriptions, etc.)
    let paymentsProcessed = 0;
    let paymentsWithOpEx = 0;
    (payments || []).forEach((payment: any) => {
      paymentsProcessed++;
      
      // Check for override on the payment itself (not line items)
      const overrideKey = `payment:${payment.qb_payment_id}:`;
      const rawIdKey = payment.qb_payment_id; // Try just the QB ID
      const override = overrideMap.get(overrideKey) || overrideMap.get(rawIdKey);

      if (override) {
        paymentsWithOpEx++;
        addToWeekly(
          payment.payment_date,
          Math.abs(parseFloat(payment.total_amount) || 0),
    ...
    console.log(`✅ [Weekly OpEx] Processed ${paymentsProcessed} payments, ${paymentsWithOpEx} had OpEx overrides`);
    ...
    console.log(`📊 [Weekly OpEx] Aggregated into ${weeklyData.length} weeks`);
    weeklyData.forEach(week => {
      console.log(`  📅 Week ${week.weekStart}: $${week.totalAmount.toFixed(2)} (${week.transactionCount} transactions)`);
      console.log(`     💼 Contract Labor: $${week.breakdown.contractLabor.toFixed(2)} | Consulting: $${week.breakdown.consultingServices.toFixed(2)} | Travel: $${week.breakdown.travel.toFixed(2)} | Software: $${week.breakdown.software.toFixed(2)}`);
    });
    ...
    console.error('❌ Error fetching weekly OpEx:', error);
    return NextResponse.json(
      { error: 'Failed to fetch weekly OpEx data' },
      { status: 500 }
    );
  }
}
